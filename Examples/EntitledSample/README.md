...
`store.updates`. The scheme runs against `Entitled.storekit`
(`pro.monthly` + `pro.yearly` in group `21000001`, `lifetime` non-consumable).

```
brew install xcodegen
xcodegen generate
open EntitledSample.xcodeproj     # ⌘R to play, ⌘U to run the tests
```

- `EntitledSampleTests` — `SKTestSession` against real StoreKit 2: purchase →
  ...
  the group entitled.
- `EntitledSampleUITests` — taps *Subscribe*, asserts the badge, expires the
