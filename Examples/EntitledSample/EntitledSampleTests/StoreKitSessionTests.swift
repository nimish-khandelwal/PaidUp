...
/// Real StoreKit 2 against the checked-in `Entitled.storekit` configuration.
...
final class StoreKitSessionTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("EntitledSampleTests-\(UUID().uuidString)")
    }
    ...
    private func makeStore(onError: (@Sendable (EntitledError) -> Void)? = nil) -> Entitled {
        var config = EntitledConfiguration.default
        config.storageDirectory = directory
        config.onError = onError
        return Entitled(products: ["pro.monthly", "pro.yearly", "lifetime"], userID: userID, configuration: config)
    }
