...
Entitled never fails silently. Most reports below are explained by one line
from this handler.
...
5. **Billing retry without a grace period.** Apple stops granting access;
   so does Entitled. Enable Billing Grace Period in App Store Connect if you
   want those days covered — they will then arrive as `.gracePeriod`.
6. **The instance was deallocated.** Create one `Entitled` and keep it alive.
...
correct for Entitled to report nothing; buy again.
...
Note that Entitled's disk cache is per-device; it never syncs anything.
...
threw or exceeded `remoteTimeout`. Entitled keeps the last good remote set,
...
transaction arrives through `Transaction.updates`, Entitled recomputes, and
