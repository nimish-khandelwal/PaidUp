...
Entitled does this for you: the `userID` you pass to `init` is attached as
...
let store = Entitled(products: products, userID: user.id, configuration: config)
```

Entitled calls the provider at `init`, when the app returns to the
...
- **Tampered device** — only `.verified` transactions count. If you want a
  second opinion, send the JWS to your server from your own code; Entitled
  does not transmit anything.
- **Clock tampering** — Entitled never compares `expirationDate` to the
